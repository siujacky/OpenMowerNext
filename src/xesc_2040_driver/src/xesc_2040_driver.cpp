...
Xesc2040Driver::Xesc2040Driver(rclcpp::Node::SharedPtr node)
: node_(node)
{
  RCLCPP_INFO(node_->get_logger(), "Starting xesc 2040 driver");

  xesc_interface_ = std::make_unique<Xesc2040Interface>(
    std::bind(&Xesc2040Driver::errorCallback, this, std::placeholders::_1));
  ...
  xesc_interface_->updateSettings(
    hall_table,
    static_cast<float>(motor_current_limit),
    static_cast<float>(acceleration),
    has_motor_temp,
    static_cast<float>(min_motor_temp),
    static_cast<float>(max_motor_temp),
    static_cast<float>(min_pcb_temp),
...
void Xesc2040Driver::getStatus(xesc_msgs::msg::XescStateStamped & state_msg)
{
  ...
  state_msg.state.connection_state =
    static_cast<uint8_t>(status_.connection_state);
