...
public:
  XbotSensorExampleNode() : Node("xbot_sensor_example"), counter_(0)
  {
    ...
    sensor_info_pub_ = this->create_publisher<xbot_msgs::msg::SensorInfo>(
        "xbot_monitoring/sensors/" + sensor_info_.sensor_id + "/info", rclcpp::QoS(1).transient_local());
    ...
    timer_ = this->create_wall_timer(1s, std::bind(&XbotSensorExampleNode::timer_callback, this));
