    ...
    XbotMonitoringNode() : Node("xbot_monitoring") {
        ...
        if (external_mqtt_enable_) {
            RCLCPP_INFO(this->get_logger(), "Using external MQTT broker: %s:%s with topic prefix: %s",
                        external_mqtt_hostname_.c_str(), external_mqtt_port_.c_str(),
                        external_mqtt_topic_prefix_.c_str());
        }
        ...
        cmd_vel_pub_ = this->create_publisher<geometry_msgs::msg::Twist>(
            "xbot_monitoring/remote_cmd_vel", 1);
        action_pub_ = this->create_publisher<std_msgs::msg::String>("xbot/action", 1);
        rpc_request_pub_ = this->create_publisher<xbot_rpc_msgs::msg::RpcRequest>(
            "xbot_rpc/request", 100);
        ...
        map_sub_ = this->create_subscription<std_msgs::msg::String>(
            "mower_map_service/json_map", 10,
            std::bind(&XbotMonitoringNode::map_callback, this, std::placeholders::_1));
        ...
        rpc_response_sub_ = this->create_subscription<xbot_rpc_msgs::msg::RpcResponse>(
            "xbot_rpc/response", 100,
            std::bind(&XbotMonitoringNode::rpc_response_callback, this, std::placeholders::_1));
        
        rpc_error_sub_ = this->create_subscription<xbot_rpc_msgs::msg::RpcError>(
            "xbot_rpc/error", 100,
            std::bind(&XbotMonitoringNode::rpc_error_callback, this, std::placeholders::_1));

        // Services
        register_actions_service_ = this->create_service<xbot_msgs::srv::RegisterActionsSrv>(
            "xbot/register_actions",
            std::bind(&XbotMonitoringNode::register_actions_callback, this,
                      std::placeholders::_1, std::placeholders::_2));

        register_methods_service_ = this->create_service<xbot_rpc_msgs::srv::RegisterMethodsSrv>(
            "xbot_rpc/register_methods",
            std::bind(&XbotMonitoringNode::register_methods_callback, this,
                      std::placeholders::_1, std::placeholders::_2));

        // Timer for sensor discovery
        sensor_check_timer_ = this->create_wall_timer(
            100ms, std::bind(&XbotMonitoringNode::sensor_check_callback, this));
...
private:
    ...
    void publish_sensor_metadata() {
        ...
        if (found_sensors_.empty()) return;
    ...
    void map_overlay_callback(const xbot_msgs::msg::MapOverlay::SharedPtr msg) {
        ...
        for (const auto& poly : msg->polygons) {
            if (poly.polygon.points.size() < 2) continue;
    ...
    void register_actions_callback(
        const std::shared_ptr<xbot_msgs::srv::RegisterActionsSrv::Request> request,
        std::shared_ptr<xbot_msgs::srv::RegisterActionsSrv::Response> /*response*/) {
        RCLCPP_INFO(this->get_logger(), "New actions registered: %s registered %zu actions",
                    request->node_prefix.c_str(), request->actions.size());
        registered_actions_[request->node_prefix] = request->actions;
        publish_actions();
    }

    void register_methods_callback(
        const std::shared_ptr<xbot_rpc_msgs::srv::RegisterMethodsSrv::Request> request,
        ...
        RCLCPP_INFO(this->get_logger(), "New methods registered: %s registered %zu methods",
                    request->node_id.c_str(), request->methods.size());
    }
