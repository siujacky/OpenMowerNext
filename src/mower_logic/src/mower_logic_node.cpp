...
public:
    MowerLogicNode() : Node("mower_logic") {
        ...
        emergency_sub_ = std::make_unique<StateSubscriber<mower_msgs::msg::Emergency>>(
            this, "/ll/emergency");
        status_sub_ = std::make_unique<StateSubscriber<mower_msgs::msg::Status>>(
            this, "/ll/mower_status");
        power_sub_ = std::make_unique<StateSubscriber<mower_msgs::msg::Power>>(
            this, "/ll/power");
        left_esc_sub_ = std::make_unique<StateSubscriber<mower_msgs::msg::ESCStatus>>(
            this, "/ll/diff_drive/left_esc_status");
        right_esc_sub_ = std::make_unique<StateSubscriber<mower_msgs::msg::ESCStatus>>(
            this, "/ll/diff_drive/right_esc_status");
        pose_sub_ = std::make_unique<StateSubscriber<xbot_msgs::msg::AbsolutePose>>(
            this, "/xbot_positioning/xb_pose");
        ...
        gps_client_ = this->create_client<xbot_positioning_msgs::srv::GPSControlSrv>(
            "xbot_positioning/set_gps_state");
        mow_client_ = this->create_client<mower_msgs::srv::MowerControlSrv>(
            "ll/_service/mow_enabled");
        emergency_client_ = this->create_client<mower_msgs::srv::EmergencyStopSrv>(
            "ll/_service/emergency");
        positioning_client_ = this->create_client<xbot_positioning_msgs::srv::SetPoseSrv>(
            "xbot_positioning/set_robot_pose");
        action_registration_client_ = this->create_client<xbot_msgs::srv::RegisterActionsSrv>(
        ...
        high_level_control_srv_ = this->create_service<mower_msgs::srv::HighLevelControlSrv>(
            ...
            std::bind(&MowerLogicNode::highLevelControlCallback, this, _1, _2));

        // Create timers
        safety_timer_ = this->create_wall_timer(
            500ms, std::bind(&MowerLogicNode::safetyTimerCallback, this));
        ui_timer_ = this->create_wall_timer(
            1s, std::bind(&MowerLogicNode::updateUI, this));
...
private:
    ...
    void registerActions(const std::string& prefix, 
                         const std::vector<xbot_msgs::msg::ActionInfo>& actions) {
    ...
    bool isGpsGood() {
        ...
        return pose.orientation_valid && 
               pose.position_accuracy < config_.max_position_accuracy &&
    ...
    void safetyTimerCallback() {
        ...
        if ((this->now() - status_time).seconds() > 3.0 ||
            (this->now() - power_time).seconds() > 3.0) {
            setEmergencyMode(true);
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 5000,
                                  "Status/power values stopped");
            return;
        }
        ...
        // Battery percentage
        double battery_percent = (power.v_battery - battery_empty_voltage_) /
        ...
        setMowerEnabled(current_behavior_ != nullptr && 
                        mower_allowed_.load() && 
                        current_behavior_->mower_enabled());
    }
    ...
    void highLevelControlCallback(
        const std::shared_ptr<mower_msgs::srv::HighLevelControlSrv::Request> request,
