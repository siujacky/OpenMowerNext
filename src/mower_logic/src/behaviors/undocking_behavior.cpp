...
void UndockingBehavior::enter() {
    ...
    docking_point_client_ = node_->create_client<mower_map_msgs::srv::GetDockingPointSrv>(
    ...
    pose_sub_ = node_->create_subscription<xbot_msgs::msg::AbsolutePose>(
        "/xbot_positioning/xb_pose", 10,
        [this](const xbot_msgs::msg::AbsolutePose::SharedPtr msg) {
            last_pose_ = *msg;
...
bool UndockingBehavior::wait_for_gps() {
    ...
    while (rclcpp::ok() && !aborted_.load()) {
        if (hasGoodGPS()) {
            ...
            if (has_pose_.load()) {
                RCLCPP_INFO(node_->get_logger(), "Waiting for GPS. Current accuracy: %f",
                    last_pose_.position_accuracy);
            } else {
    ...
    std::this_thread::sleep_for(
        std::chrono::duration<double>(undock_config_.gps_wait_time));
...
Behavior* UndockingBehavior::execute() {
    ...
    for (int i = 0; i < angled_undock_point_count; i++) {
        double orientation = yaw + angle * 
            (undock_config_.undock_use_curve ? ((i + 1.0) / angled_undock_point_count) : 1.0);
