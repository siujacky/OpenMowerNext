...
void MowingBehavior::enter()
{
  ...
  map_client_ = node_->create_client<mower_map_msgs::srv::GetMowingAreaSrv>("mower_map_service/get_mowing_area");
  path_client_ = node_->create_client<slic3r_coverage_planner_msgs::srv::PlanPath>("slic3r_coverage_planner/plan_path");
  path_progress_client_ =
      node_->create_client<ftc_local_planner_msgs::srv::PlannerGetProgress>("ftc_local_planner/get_progress");
  set_nav_point_client_ = node_->create_client<mower_map_msgs::srv::SetNavPointSrv>("mower_map_service/set_nav_point");
  clear_nav_point_client_ =
      node_->create_client<mower_map_msgs::srv::ClearNavPointSrv>("mower_map_service/clear_nav_point");
...
void MowingBehavior::reset()
{
  ...
  current_mowing_angle_increment_sum_ =
      std::fmod(current_mowing_angle_increment_sum_ + mow_config_.mow_angle_increment, 360.0);
  checkpoint();
...
bool MowingBehavior::create_mowing_plan(int area_index)
{
  ...
  double mow_angle_offset =
      std::fmod(mow_config_.mow_angle_offset + current_mowing_angle_increment_sum_ + 180.0, 360.0);
  if (mow_angle_offset < 0)
    mow_angle_offset += 360.0;
  ...
  {
    RCLCPP_INFO(node_->get_logger(), "MowingBehavior: Advancing to checkpoint, path: %d index: %d",
                current_mowing_path_, current_mowing_path_index_);
  }
  else
  {
    RCLCPP_INFO(node_->get_logger(), "MowingBehavior: Ignoring checkpoint, plan changed");
...
int MowingBehavior::get_current_mow_path_index()
{
  ...
  {
    RCLCPP_ERROR(node_->get_logger(), "MowingBehavior: Error getting progress from planner");
    return -1;
  }
...
bool MowingBehavior::execute_mowing_plan()
{
  ...
  while (static_cast<size_t>(current_mowing_path_) < current_mowing_paths_.size() && rclcpp::ok() && !aborted_.load())
  {
    // PAUSE HANDLING
    if (requested_pause_flag_.load())
    {
      ...
      while (requested_pause_flag_.load() && !aborted_.load())
      {
        ...
        RCLCPP_INFO_THROTTLE(node_->get_logger(), *node_->get_clock(), 30000, "MowingBehavior: PAUSED (%s)",
                             pause_reason.c_str());
    ...
    RCLCPP_INFO(node_->get_logger(), "MowingBehavior: Path segment length: %zu poses", path.path.poses.size());
    ...
    // DRIVE TO FIRST POINT
    {
      ...
      if (!navigation_succeeded && !requested_pause_flag_.load() && !skip_path_.load())
      {
        ...
        if (first_point_attempt_counter < mow_config_.max_first_point_attempts)
        {
          RCLCPP_WARN(node_->get_logger(), "MowingBehavior: (FIRST POINT) Attempt %d/%d failed",
        ...
        {
          RCLCPP_WARN(node_->get_logger(), "MowingBehavior: (FIRST POINT) Trimming start point, attempt %d/%d",
        ...
        {
          RCLCPP_ERROR(node_->get_logger(), "MowingBehavior: Max retries reached, aborting");
          abort();
        }
        continue;
      }
    ...
    // EXECUTE MOW PATH
    {
      ...
      RCLCPP_INFO(node_->get_logger(), "MowingBehavior: (MOW) Executing path with %zu poses from index %d",
      ...
      while (rclcpp::ok())
      {
        ...
        if (status == rclcpp_action::GoalStatus::STATUS_EXECUTING)
        {
          ...
          RCLCPP_INFO_THROTTLE(node_->get_logger(), *node_->get_clock(), 5000, "MowingBehavior: (MOW) Progress: %d/%zu",
      ...
      if (path_succeeded || current_mowing_path_index_ >= static_cast<int>(path.path.poses.size()) ||
      ...
      else if (!requested_pause_flag_.load())
      {
        ...
        RCLCPP_INFO(node_->get_logger(), "MowingBehavior: (MOW) PAUSED at index %d", current_mowing_path_index_);
        paused_.store(true);
        update_actions();
      }
    }
