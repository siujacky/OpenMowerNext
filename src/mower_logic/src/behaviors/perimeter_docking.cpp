...
{
  // Create subscriber
  perimeter_sub_ = node_->create_subscription<mower_msgs::msg::Perimeter>("/mower/perimeter", rclcpp::SensorDataQoS(),
                                                                          perimeter_received);

  // Create service client
  perimeter_client_ = node_->create_client<mower_msgs::srv::PerimeterControlSrv>("/mower_service/perimeter_listen");
...
{
  return direction_ > 0 ? last_perimeter_.left * calibration_left_ : last_perimeter_.right * calibration_right_;
}

float PerimeterBase::outer_signal()
{
  return direction_ > 0 ? last_perimeter_.right * calibration_right_ : last_perimeter_.left * calibration_left_;
}
...
Behavior* PerimeterSearchBehavior::execute()
{
  ...
  calibration_left_ = calibration_right_ = sign_center_ = last_perimeter_.left < 0 ? 1 : -1;
  ...
  while (tries-- > 0)
  {
    ...
    {
      tries = 10;
      if (inner_signal() > 0 || outer_signal() > 0)
        break;
    }
  }
...
Behavior* PerimeterDockingBehavior::arrived()
{
  ...
  {
    RCLCPP_WARN(node_->get_logger(), "Travelled %.0f meters before reaching the station", travelled_);
    return &IdleBehavior::INSTANCE;
  }
...
Behavior* PerimeterMoveToGpsBehavior::arrived()
{
  ...
  return travelled_ >= perimeter_config_.undock_distance - 0.9 ? &MowingBehavior::INSTANCE : nullptr;
}
