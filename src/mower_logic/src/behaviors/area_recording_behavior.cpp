...
void AreaRecordingBehavior::enter()
{
  ...
  add_mowing_area_client_ =
      node_->create_client<mower_map_msgs::srv::AddMowingAreaSrv>("mower_map_service/add_mowing_area");
  set_docking_point_client_ =
      node_->create_client<mower_map_msgs::srv::SetDockingPointSrv>("mower_map_service/set_docking_point");

  // Create publishers
  marker_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>("area_recorder/progress_visualization", 10);
  map_overlay_pub_ = node_->create_publisher<xbot_msgs::msg::MapOverlay>("xbot_monitoring/map_overlay", 10);
  marker_array_pub_ =
      node_->create_publisher<visualization_msgs::msg::MarkerArray>("area_recorder/progress_visualization_array", 10);
  ...
  auto_point_collecting_sub_ = node_->create_subscription<std_msgs::msg::Bool>(
      "/record_auto_point_collecting", 100, std::bind(&AreaRecordingBehavior::record_auto_point_collecting, this, _1));
...
void AreaRecordingBehavior::joy_received(const sensor_msgs::msg::Joy::SharedPtr msg)
{
  ...
  if (!msg->buttons.empty() && msg->buttons.size() > 1 && msg->buttons[1] && !last_joy_.buttons.empty() &&
      !last_joy_.buttons[1])
  {
  ...
  if (!msg->buttons.empty() && msg->buttons.size() > 2 && msg->buttons[2] && !last_joy_.buttons.empty() &&
      !last_joy_.buttons[2])
  {
  ...
  if (!msg->buttons.empty() && msg->buttons.size() > 5 && msg->buttons[5] && !last_joy_.buttons.empty() &&
      !last_joy_.buttons[5])
  {
    if (msg->buttons.size() > 4 && msg->buttons[4])
    {
      ...
      RCLCPP_INFO(node_->get_logger(), "Auto point collecting: %s", auto_point_collecting_.load() ? "true" : "false");
    }
    else
    {
...
void AreaRecordingBehavior::record_auto_point_collecting(const std_msgs::msg::Bool::SharedPtr msg)
{
  ...
  RCLCPP_INFO(node_->get_logger(), "Recording auto point collecting: %s", msg->data ? "enabled" : "disabled");
}
...
bool AreaRecordingBehavior::get_docking_position(geometry_msgs::msg::Pose& pos)
{
  ...
  else
  {
    ...
    double yaw =
        std::atan2(pos.position.y - first_docking_pos_.position.y, pos.position.x - first_docking_pos_.position.x);
                                               ...
                                               xbot_msgs::msg::MapOverlay& result_overlay)
{
  ...
  while (true)
  {
    ...
    else
    {
      ...
      bool is_point_manual_collected =
          !auto_point_collecting_.load() && collect_point_.load() && is_new_point_far_enough;
  ...
  while (rclcpp::ok() && !aborted_.load())
  {
    ...
    while (rclcpp::ok() && !finished_all_.load() && !error && !aborted_.load())
    {
      if (set_docking_position_.load())
      {
        ...
        {
          RCLCPP_INFO(node_->get_logger(), "New docking pos = (%.2f, %.2f)", pos.position.x, pos.position.y);
