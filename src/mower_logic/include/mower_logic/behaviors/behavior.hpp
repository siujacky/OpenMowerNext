...
enum class AutoMode { MANUAL = 0, SEMIAUTO = 1, AUTO = 2 };
...
public:
    ...
    bool hasGoodGPS() const { return is_gps_good_.load(); }
    void setGoodGPS(bool is_good) { is_gps_good_.store(is_good); }
    ...
    void start(const MowerLogicConfig& config, std::shared_ptr<SharedState> state, 
               rclcpp::Node* node) {
