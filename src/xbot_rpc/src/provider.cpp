...
void RpcProvider::init(rclcpp::Node::SharedPtr node)
{
  ...
  response_pub_ = node_->create_publisher<xbot_rpc_msgs::msg::RpcResponse>(
    TOPIC_RESPONSE, qos);

  error_pub_ = node_->create_publisher<xbot_rpc_msgs::msg::RpcError>(
    TOPIC_ERROR, qos);

  registration_client_ = node_->create_client<xbot_rpc_msgs::srv::RegisterMethodsSrv>(
    SERVICE_REGISTER_METHODS);

  // Wait for registration service
  if (!registration_client_->wait_for_service(10s)) {
    RCLCPP_WARN(node_->get_logger(),
      "Registration service not available after waiting");
  }
...
void RpcProvider::handleRequest(const xbot_rpc_msgs::msg::RpcRequest::SharedPtr request)
{
  ...
  if (!request->params.empty()) {
    try {
      ...
      publishError(
        request,
        xbot_rpc_msgs::msg::RpcError::ERROR_INVALID_JSON,
  ...
  try {
    ...
    publishError(
      request,
      xbot_rpc_msgs::msg::RpcError::ERROR_INTERNAL,
      std::string("Internal error: ") + e.what());
  }
}

void RpcProvider::publishResponse(
