...
protected:
  ...
  void debugObstacle(visualization_msgs::msg::Marker& obstacle_points, double x, double y, unsigned char cost,
                     int max_ids);
  double timeInCurrentState();

  void progressServiceCallback(const std::shared_ptr<ftc_local_planner_msgs::srv::PlannerGetProgress::Request> request,
