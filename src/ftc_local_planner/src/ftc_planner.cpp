...
void FTCPlanner::configure(const rclcpp_lifecycle::LifecycleNode::WeakPtr& parent, std::string name,
                           ...
                           std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  // Create publishers
  global_point_pub_ = node->create_publisher<geometry_msgs::msg::PoseStamped>(plugin_name_ + "/global_point", 1);
  global_plan_pub_ =
      node->create_publisher<nav_msgs::msg::Path>(plugin_name_ + "/global_plan", rclcpp::QoS(1).transient_local());
  obstacle_marker_pub_ = node->create_publisher<visualization_msgs::msg::Marker>(plugin_name_ + "/costmap_marker", 10);

  if (config_.debug_pid)
  {
    pid_pub_ = node->create_publisher<ftc_local_planner_msgs::msg::PID>(plugin_name_ + "/debug_pid",
  ...
  progress_service_ = node->create_service<ftc_local_planner_msgs::srv::PlannerGetProgress>(
      ...
      std::bind(&FTCPlanner::progressServiceCallback, this, std::placeholders::_1, std::placeholders::_2));

  // Initialize oscillation detector
  failure_detector_.setBufferLength(static_cast<size_t>(std::round(config_.oscillation_recovery_min_duration * 10)));
...
void FTCPlanner::loadParameters()
{
  ...
  nav2_util::declare_parameter_if_not_declared(node, plugin_name_ + ".speed_fast_threshold",
                                               rclcpp::ParameterValue(0.3));
  nav2_util::declare_parameter_if_not_declared(node, plugin_name_ + ".speed_fast_threshold_angle",
  ...
  nav2_util::declare_parameter_if_not_declared(node, plugin_name_ + ".max_goal_distance_error",
                                               rclcpp::ParameterValue(0.05));
  nav2_util::declare_parameter_if_not_declared(node, plugin_name_ + ".max_goal_angle_error",
                                               rclcpp::ParameterValue(5.0));
  nav2_util::declare_parameter_if_not_declared(node, plugin_name_ + ".max_follow_distance",
  ...
  nav2_util::declare_parameter_if_not_declared(node, plugin_name_ + ".obstacle_footprint",
  ...
  nav2_util::declare_parameter_if_not_declared(node, plugin_name_ + ".oscillation_recovery",
                                               rclcpp::ParameterValue(true));
  nav2_util::declare_parameter_if_not_declared(node, plugin_name_ + ".oscillation_recovery_min_duration",
                                               rclcpp::ParameterValue(2.0));
  nav2_util::declare_parameter_if_not_declared(node, plugin_name_ + ".oscillation_v_eps", rclcpp::ParameterValue(0.1));
  nav2_util::declare_parameter_if_not_declared(node, plugin_name_ + ".oscillation_omega_eps",
...
void FTCPlanner::setPlan(const nav_msgs::msg::Path& path)
{
  ...
  if (global_plan_.size() > 2)
  {
    ...
    global_plan_[global_plan_.size() - 2].pose.orientation = global_plan_[global_plan_.size() - 3].pose.orientation;
  }
  else
  {
...
geometry_msgs::msg::TwistStamped FTCPlanner::computeVelocityCommands(const geometry_msgs::msg::PoseStamped& pose,
...
double FTCPlanner::distanceLookahead()
{
  ...
  for (uint32_t i = current_index_ + 1; i < global_plan_.size(); i++)
  {
    ...
    if (lookahead_distance > config_.speed_fast_threshold ||
        std::abs(rot2.angularDistance(current_rot)) > config_.speed_fast_threshold_angle * (M_PI / 180.0))
    {
      break;
    }
...
FTCPlanner::PlannerState FTCPlanner::updatePlannerState()
{
  switch (current_state_)
  {
    ...
    case PlannerState::FOLLOWING: {
      ...
      {
        RCLCPP_ERROR(logger_, "FTCPlanner: Robot far from path (%.2f > %.2f)", distance, config_.max_follow_distance);
        is_crashed_ = true;
        return PlannerState::FINISHED;
      }
...
void FTCPlanner::updateControlPoint(double dt)
{
  ...
  switch (current_state_)
  {
    ...
    case PlannerState::FOLLOWING: {
      ...
      double speed = (straight_dist >= config_.speed_fast_threshold) ? config_.speed_fast : config_.speed_slow;

      if (speed > current_movement_speed_)
      {
        current_movement_speed_ += dt * config_.acceleration;
        if (current_movement_speed_ > speed)
          current_movement_speed_ = speed;
      }
      ...
      result.translation() =
          (1.0 - current_progress_) * currentPose.translation() + current_progress_ * nextPose.translation();
  ...
  try
  {
    auto map_to_base = tf_buffer_->lookupTransform("base_link", "map", tf2::TimePointZero);
...
void FTCPlanner::calculateVelocityCommands(double dt, geometry_msgs::msg::TwistStamped& cmd_vel)
{
  ...
  {
    double lin_speed = lon_error_ * config_.kp_lon + i_lon_error_ * config_.ki_lon + d_lon * config_.kd_lon;
...
bool FTCPlanner::checkOscillation(geometry_msgs::msg::TwistStamped& cmd_vel)
{
  ...
  failure_detector_.update(cmd_vel, config_.max_cmd_vel_speed, config_.max_cmd_vel_speed, config_.max_cmd_vel_ang,
